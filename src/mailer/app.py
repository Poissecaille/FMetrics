...
def mail_sender():
    ...
    try:
        if type(request.json) is dict:
            ...
            if "uuid" in json_keys_list:
                ...
                if result.status_code == 200:
                    logger_info.info(
                        {"message": f"A mail was sent to customer {user_address} to reset his password! with uuid {uuid}"}, extra={"level": "INFO", "method": "POST", "endpoint": "/MAIL"})
            ...
            if "registration" in json_keys_list:
                ...
                if result.status_code == 200:
                    logger_info.info(
                        {"message": f"A mail was sent to customer {user_address} to bid him welcome with uuid {random_uuid_register}!"}, extra={"level": "INFO", "method": "POST", "endpoint": "/MAIL"})
                return Response({"response": "mail sent to {}".format(user_address)}, status=result.status_code, mimetype='application/json')
            ...
            if "accept" in json_keys_list:
                ...
                if result.status_code == 200:
                    logger_info.info(
                        {"message": f"A mail was sent to customer {user_address} to confirm the ckecking of his address!"}, extra={"level": "INFO", "method": "POST", "endpoint": "/MAIL"})
                return Response({"response": "mail sent to {}".format(user_address)}, status=result.status_code, mimetype='application/json')
            ...
            if "secret" in json_keys_list:
                ...
                if result.status_code == 200:
                    logger_info.info(
                        {"message": f"A mail was sent to customer {user_address}. The customer {sender} accepted his friend request by sharing his secret: {secret}!"}, extra={"level": "INFO", "method": "POST", "endpoint": "/MAIL"})
                return Response({"response": "mail sent to {}".format(user_address)}, status=result.status_code, mimetype='application/json')
            ...
            if "expiration_date" in json_keys_list:
                ...
                if result.status_code == 200:
                    logger_info.info(
                        {"message": f"A mail was sent to customer {user_address} to notify that his alert {alert_name} has expired!"}, extra={"level": "INFO", "method": "POST", "endpoint": "/MAIL"})
                    return Response({"response": "mail sent to {}".format(user_address)}, status=result.status_code, mimetype='application/json')
        ...
        if result.status_code == 200:
            logger_info.info(
                {"message": f"A mail was sent to customer {user_address} to notify that his alert {alert_name} has been triggered!"}, extra={"level": "INFO", "method": "POST", "endpoint": "/MAIL"})
            return Response({"response": "mail sent to {}".format(user_address)}, status=result.status_code, mimetype='application/json')
    except Exception as e:
        logger_error.error(
            {"message": f"An error occured! - {e}"}, extra={"level": "ERROR", "method": "POST", "endpoint": "/MAIL"})
