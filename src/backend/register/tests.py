...
from register.models import BlockedUsers
...
class RelationalSystemTest(TestCase):
    ...
    def create_test_user(self, email, password, visibility):
        json_data = {"email": email, "password": password,
                     "skill": "advanced", "visibility": visibility}
        response = self.client.post(
            'http://127.0.0.1:$BACKEND_PORT/register/', data=json.dumps(json_data), content_type='application/json')
        return json.loads(response.content.decode("utf-8"))["token"]
    ...
    def test_no_data(self):
        ...
        email = email.split(' ')[0].replace('.', '') + \
            email.split(' ')[1].replace('.', '')+'@etna-alternance.net'
        password = self.generate_random_password(20)
        visibility = self.visibility_levels[1]
        token = self.create_test_user(email, password, visibility)
        response = self.client.post(
    ...
    def test_wrong_request(self):
        ...
        password = self.generate_random_password(20)
        visibility = self.visibility_levels[1]
        self.create_test_user(email, password, visibility)
    ...
    def test_ask_to_private_his_secret(self):
        email1 = self.fake.unique.name()
        email1 = email1.split(' ')[0].replace('.', '') + \
            email1.split(' ')[1].replace('.', '')+'@etna-alternance.net'
        password = self.generate_random_password(20)
        visibility = self.visibility_levels[0]
        token1 = self.create_test_user(email1, password, visibility)
        user1 = CustomUser.objects.get(email=email1)
        ...
        password = self.generate_random_password(20)
        self.create_test_user(email2, password, visibility)
        user2 = CustomUser.objects.get(email=email2)
    ...
    def test_follow_public_with_secret(self):
        email1 = self.fake.unique.name()
        email1 = email1.split(' ')[0].replace('.', '') + \
            email1.split(' ')[1].replace('.', '')+'@etna-alternance.net'
        password = self.generate_random_password(20)
        ...
        self.assertEqual(response.status_code, 403)
        self.assertEqual([relation for relation in FollowUser.objects.filter(
            follower_id=user1.id, followed_id=user2.id)], [])

    def test_follow_protected_with_pseudo(self):
        email1 = self.fake.unique.name()
        email1 = email1.split(' ')[0].replace('.', '') + \
            email1.split(' ')[1].replace('.', '')+'@etna-alternance.net'
        password = self.generate_random_password(20)
    ...
    def test_correct_public_follow(self):
        email1 = self.fake.unique.name()
        email1 = email1.split(' ')[0].replace('.', '') + \
            email1.split(' ')[1].replace('.', '')+'@etna-alternance.net'
        password = self.generate_random_password(20)
        visibility = self.visibility_levels[2]
        token1 = self.create_test_user(email1, password, visibility)
        user1 = CustomUser.objects.get(email=email1)
    ...
    def test_correct_protected_follow(self):
        email1 = self.fake.unique.name()
        email1 = email1.split(' ')[0].replace('.', '') + \
            email1.split(' ')[1].replace('.', '')+'@etna-alternance.net'
        password = self.generate_random_password(20)
        ...
        password = self.generate_random_password(20)
        visibility = "protected"
        token2 = self.create_test_user(email2, password, visibility)
        user2 = CustomUser.objects.get(email=email2)
        json_data = {"pseudo": user2.pseudo}
        response = self.client.post(
            'http://127.0.0.1:$BACKEND_PORT/ask_secret/', data=json.dumps(json_data), content_type='application/json', HTTP_AUTHORIZATION='Token {}'.format(token1))
    ...
    def test_followed_and_followers(self):
        email1 = self.fake.unique.name()
        email1 = email1.split(' ')[0].replace('.', '') + \
            email1.split(' ')[1].replace('.', '')+'@etna-alternance.net'
        password = self.generate_random_password(20)
        ...
        self.assertEqual(json.loads(response.content)
                         [0]["pseudo"], user2.pseudo)
        response = self.client.get(
            'http://127.0.0.1:$BACKEND_PORT/get_followers/', content_type='application/json', HTTP_AUTHORIZATION='Token {}'.format(token1))
        self.assertEqual(json.loads(response.content), [])

    def test_user_blocks_himself(self):
        email1 = self.fake.unique.name()
        email1 = email1.split(' ')[0].replace('.', '') + \
            email1.split(' ')[1].replace('.', '')+'@etna-alternance.net'
        password = self.generate_random_password(20)
        ...
        json_data = {"pseudo": user1.pseudo}
        response = self.client.post(
            'http://127.0.0.1:$BACKEND_PORT/block_user/', data=json.dumps(json_data), content_type='application/json', HTTP_AUTHORIZATION='Token {}'.format(token1))
        self.assertEqual(response.status_code, 400)

    def test_block_user_fails_no_relation(self):
        email1 = self.fake.unique.name()
        email1 = email1.split(' ')[0].replace('.', '') + \
            email1.split(' ')[1].replace('.', '')+'@etna-alternance.net'
        password = self.generate_random_password(20)
    ...
    def test_correct_block_user(self):
        ...
        json_data = {"pseudo": user1.pseudo}
        response = self.client.post(
            'http://127.0.0.1:$BACKEND_PORT/block_user/', data=json.dumps(json_data), content_type='application/json', HTTP_AUTHORIZATION='Token {}'.format(token2))
        self.assertEqual(response.status_code, 200)
        self.assertTrue(BlockedUsers.objects.get(
            blocker_user_id=user2.id, blocked_user_id=user1.id
        ))
        self.assertEqual([relation for relation in FollowUser.objects.filter(
            follower_id=user1.id, followed_id=user2.id)], [])
    ...
    def test_unblock_unblocked_user(self):
        ...
        response = self.client.delete(
            'http://127.0.0.1:$BACKEND_PORT/unblock_user/', data=json.dumps(json_data), content_type='application/json', HTTP_AUTHORIZATION='Token {}'.format(token2))
        self.assertEqual(response.status_code, 400)
    ...
    def test_correct_unblock_user(self):
        ...
        response = self.client.delete(
            'http://127.0.0.1:$BACKEND_PORT/unblock_user/', data=json.dumps(json_data), content_type='application/json', HTTP_AUTHORIZATION='Token {}'.format(token2))
        self.assertEqual(response.status_code, 200)
        self.assertEqual([relation for relation in BlockedUsers.objects.filter(
            blocker_user_id=user2.id, blocked_user_id=user1.id
        )], [])
