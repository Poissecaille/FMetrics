...
def ask_secret(request):
    ...
    try:
        if request.method == "POST":
            ...
            if asked_user.visibility == "private":
                logger_error.error(
                    f'The user {asked_user.pseudo} was not allowed to ask the secret of the user {asked_user.pseudo} - it is a private account!',
                    extra={"level": "ERROR", "method": "POST", "endpoint": "/ASK_SECRET"})
        ...
        logger_error.error(
            'A user failed to ask the secret of another one - lack of data integrity!',
            extra={"level": "ERROR", "method": "POST", "endpoint": "/ASK_SECRET"})
        return Response(data={"response:": "invalid json format"}, status=status.HTTP_400_BAD_REQUEST)
    except CustomUser.DoesNotExist:
        logger_error.error(
            'A user failed to ask the secret of another one - target does not exist!',
            extra={"level": "ERROR", "method": "POST", "endpoint": "/ASK_SECRET"})
